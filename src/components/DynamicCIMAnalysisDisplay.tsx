...
export function DynamicCIMAnalysisDisplay({ dealId }: DynamicCIMAnalysisDisplayProps) {
  ...
  const fetchCIMAnalysis = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        try {
          // Scan and organize the data dynamically with error handling
          ...
          console.error('Error during data scanning:', scanError);
          // Continue with basic display even if scanning fails
          setSections([]);
          setUnknownFields([]);
        }
      } else {
  ...
  const handleFieldEdit = async (field: any, newLabel: string) => {
    try {
      // For now, just update the local display without external service calls
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-center py-8">
            ...
            <Button onClick={fetchCIMAnalysis}>
